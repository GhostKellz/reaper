...
pub const PhantomTui = struct {
    ...
    fn installSelectedPackage(self: *PhantomTui) !void {
        ...
        const task_id = try std.fmt.allocPrint(
            self.allocator,
            "install_{s}_{d}",
            .{ package_entry.package.name, std.time.timestamp() }
        );
        
        const description = try std.fmt.allocPrint(
            self.allocator,
            "Installing {s}...",
            .{package_entry.package.name}
        );
        ...
        const install_task = try self.runtime.spawn(struct {
            fn install(tui: *PhantomTui, entry: *PackageEntry, bg_task_id: []const u8) !void {
                ...
                const cmd = switch (entry.source) {
                    .aur, .chaotic_aur => try std.fmt.allocPrint(
                        tui.allocator,
                        "yay -S --noconfirm {s}",
                        .{entry.package.name}
                    ),
                    .pacman => try std.fmt.allocPrint(
                        tui.allocator,
    ...
    fn refreshPackageList(self: *PhantomTui) !void {
        ...
        for (self.packages.items) |entry| {
            ...
            const item_text = try std.fmt.allocPrint(
                self.allocator,
                "{s} {s} {s} {s} - {s}",
                .{ icon, status_icon, entry.package.name, entry.package.version, entry.package.description }
            );
