...
**Reaper** is a blazing-fast, Zig-powered **AUR helper and meta package manager** for Arch Linux. Built with Zig 0.15 dev and the zsync async runtime, it provides unparalleled performance and memory safety. This is the modern Zig implementation - for the original Rust version, see [reaper-rs](https://github.com/GhostKellz/reaper-rs).
...
- **Zero-Cost Abstractions**: Performance comparable to C with modern language features  
- **Native Async**: Built-in async/await with the zsync runtime for optimal I/O
- **Cross-Platform**: Built-in cross-compilation support for all architectures
- **Predictable Performance**: No hidden allocations or unexpected performance coststtps://img.shields.io/badge/platform-Arch%20Linux-1793d1?logo=arch-linux&logoColor=white)](https://archlinux.org)
...
* Plugin/hook support with native Zig performance
* Search and PKGBUILD caching for speed
* **No yay/paru fallback** – all install/upgrade logic is native async Zig with zsync runtime
...
- All install/upgrade flows are now async/parallel using zsync runtime and do not use yay/paru fallback.
...
- See [MAKE_COMMANDS.md](MAKE_COMMANDS.md) for the full updated command list.
...
- **No yay/paru fallback: all logic is native Zig with zsync async runtime.**
