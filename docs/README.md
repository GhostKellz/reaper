# Reaper Documentation

## Getting Started

- [Quick Start](getting-started/quickstart.md) - Get up and running
- [Installation](getting-started/installation.md) - Build and install from source

## Usage

- [Commands](usage/commands.md) - CLI command reference
- [Configuration](usage/configuration.md) - Configuration options
- [Profiles](usage/profiles.md) - Multi-profile management
- [Rollback](usage/rollback.md) - Transaction-based rollback
- [Tap Publishing](usage/tap-publishing.md) - Publishing tap packages

## Security

- [Trust Model](security/trust-model.md) - Advisory trust system
- [GPG Verification](security/gpg-verification.md) - Signature verification
- [Accepted Advisories](advisories/accepted.md) - Currently accepted RustSec advisories
- [Resolved Advisories](advisories/resolved.md) - RustSec advisories cleared by dependency work

## Development

- [Roadmap](roadmap.md) - Current project priorities
- [Architecture](development/architecture.md) - System design
- [API Reference](development/api.md) - API documentation
- [Development Guide](development/dev-guide.md) - Development tips
